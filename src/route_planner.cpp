...
void RoutePlanner::AddNeighbors(RouteModel::Node *current_node) {
    ...
    for (auto neymar : current_node->neighbors){
        neymar->parent = current_node;
        neymar->h_value = CalculateHValue(neymar);
        neymar->g_value = current_node->g_value + current_node->distance(*neymar);
        open_list.push_back(neymar);
        neymar->visited = true;
...
bool fValue(RouteModel::Node *n1, RouteModel::Node *n2){
    
    float f_n1 = n1->g_value + n1->h_value;
    float f_n2 = n2->g_value + n2->h_value;
